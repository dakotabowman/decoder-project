...
describe("caesar", () => {
  it("should return the decoded message depending on the given shift value", () => {
        const actual = caesarModule.caesar("Bpqa qa iv mvkwlml umaaiom!", 8, false);
    const expected = "this is an encoded message!"; 
        expect(actual).to.eql(expected);
  });
  it("should return the encoded message depending on the given shift value", () => {
        const actual = caesarModule.caesar("this is an encoded message!", 8);
    const expected = "bpqa qa iv mvkwlml umaaiom!";
    expect(actual).to.eql(expected);
  });
